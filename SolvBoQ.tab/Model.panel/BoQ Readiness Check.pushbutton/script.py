# -*- coding: utf-8 -*-
"""
model_boq_readiness_check.py
============================
Stage 1.5 of the SANS 1200 BoQ pipeline — the "BoQ Readiness Check" button.

Runs inside Revit *before* mapping. Reads the open model and scores how
"BoQ-ready" it is (0-100): can we trust the takeoff and its classification?
Shows an HTML score card and writes a two-section CSV — a QS summary plus a
modeler fix-it list (element id + location).
...
This is a sibling of `model_health_check.py`. The Model Health Check is the
comprehensive tool; this one is a focused SUBSET — only the checks that affect
the BoQ takeoff or its classification (quantities, double-counts, scope, SANS
mapping). Modeling-hygiene checks that don't change the numbers — Mark
population and level association/cleanup — are intentionally left to the Model
Health Check. Each is fully self-contained — own model reader, no shared module.
...
REPORT_KIND = "boq_readiness_check"
...
# BoQ-readiness subset only: each check here affects the takeoff quantity or its
# SANS classification. (Mark/level hygiene lives in the Model Health Check.)
PENALTY_WEIGHTS = {
    "model_warning": 6,
    "no_material": 8,
    "zero_quantity": 6,
...
def _band(score):
    if score >= 85:
        return _GREEN, "BoQ-ready"
    if score >= 65:
        return _AMBER, "Needs attention"
    return _RED, "Not BoQ-ready"
...
def _write_report_csv(path, model_title, score, band_label, total_elements,
    ...
    row([u"SANS BoQ - BoQ Readiness Check"])
...
def _score_card_html(score, color, band_label, stats, breakdown):
    ...
    hero_html = (
        ...
        u"opacity:0.75;'>BoQ readiness check</div>"
...
def main():
    output.print_md("# BoQ readiness check")
    ...
    # --- model-level check: quantity-relevant Revit warnings (double-count risk) ---
    ...
    fname = "boq_readiness_{0}_{1}.csv".format(
    ...
    output.print_md("---")
    output.print_md("BoQ readiness check is advisory only — it never blocks extraction "
                    "or mapping. Score **{0}/100 ({1})** recorded in the report above."
